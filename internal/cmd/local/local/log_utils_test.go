...
2024-09-12 15:56:31 [1;31mERROR[m i.a.b.Application(main):25 - Unable to bootstrap Sumboard environment.
io.sumboard.db.init.DatabaseInitializationException: Database availability check failed.
	at io.sumboard.db.init.DatabaseInitializer.initialize(DatabaseInitializer.java:54) ~[io.sumboard.sumboard-db-db-lib-0.64.3.jar:?]
	at io.sumboard.bootloader.Bootloader.initializeDatabases(Bootloader.java:229) ~[io.sumboard-sumboard-bootloader-0.64.3.jar:?]
	at io.sumboard.bootloader.Bootloader.load(Bootloader.java:104) ~[io.sumboard-sumboard-bootloader-0.64.3.jar:?]
	at io.sumboard.bootloader.Application.main(Application.java:22) [io.sumboard-sumboard-bootloader-0.64.3.jar:?]
Caused by: io.sumboard.db.check.DatabaseCheckException: Unable to connect to the database.
	at io.sumboard.db.check.DatabaseAvailabilityCheck.check(DatabaseAvailabilityCheck.java:40) ~[io.sumboard.sumboard-db-db-lib-0.64.3.jar:?]
	at io.sumboard.db.init.DatabaseInitializer.initialize(DatabaseInitializer.java:45) ~[io.sumboard.sumboard-db-db-lib-0.64.3.jar:?]
	... 3 more
...
func TestJavaLogScanner(t *testing.T) {
	...
	expectLogLine("ERROR", "i.a.b.Application(main):25 - Unable to bootstrap Sumboard environment.")
	expectLogLine("ERROR", "io.sumboard.db.init.DatabaseInitializationException: Database availability check failed.")
	expectLogLine("ERROR", "Caused by: io.sumboard.db.check.DatabaseCheckException: Unable to connect to the database.")
...
func TestLastErrorLog(t *testing.T) {
	...
	expect := "Caused by: io.sumboard.db.check.DatabaseCheckException: Unable to connect to the database."
	if l != expect {
