...
# at the same time
DIR_TMP=${DIR_TMP:-$(mktemp -d -t "sbctl_install.XXXX")}
...
_trap() {
    ...
    if [ "$rv" -ne 0 ]; then
        ...
        _telemetry_event sbctl_install failed "$TELEMETRY_MSG"
        
        echo
        echo "sbctl install failed: $TELEMETRY_MSG"
    else
        _telemetry_event sbctl_install succeeded "$TELEMETRY_MSG"

        echo
        echo "sbctl install succeeded. Run:"
        echo
        echo "   sbctl --help"
        echo
...
TELEMETRY_STORE=${TELEMETRY_STORE:-~/.sumboard/analytics.yml}
...
_telemetry_init() {
    ...
    if [ -z "$TELEMETRY_INSTANCE_ID" ]; then
        ...
        mkdir -p ~/.sumboard
        echo "# This file is used by Sumboard to track anonymous usage statistics." > "$TELEMETRY_STORE"
        echo "# For more information or to opt out, please see" >> "$TELEMETRY_STORE"
        echo "# - https://docs.sumboard.com/operator-guides/telemetry" >> "$TELEMETRY_STORE"
...
_install_binary() {
    ...
    local release_data=$(_curl "https://api.github.com/repos/airbytehq/sbctl/$url_fragment")
    ...
    echo "Downloading sbctl from $release_url"
    ...
    (
        ...
        local binary=$(ls -1 release/*/sbctl | head -n 1)
...
_install_darwin() {
    ...
    elif brew ls --version sbctl > /dev/null; then
        brew update
        brew upgrade sbctl
    else
...
main() {
    ...
    _telemetry_event sbctl_install started
