...
func LocateLatestChart(chartVersion, chartFlag string) string {
	pterm.Debug.Printf("getting helm chart %q with version %q\n", common.SbChartName, chartVersion)
	...
	// Helm will consider a local directory path named "sumboard/sumboard" to be a chart repo,
	...
	//
	// Here we avoid that problem by figuring out the full URL of the sumboard chart,
	...
	if chartVersion == "" {
		if url, err := getLatestChartUrlFromRepoIndex(common.SbRepoName, common.SbRepoURL); err == nil {
			pterm.Debug.Printf("determined latest chart url: %s\n", url)
			return url
		} else {
			pterm.Debug.Printf("error determining latest chart, falling back to default behavior: %s\n", err)
		}
	}

	return common.SbChartName
}

func getLatestChartUrlFromRepoIndex(repoName, repoUrl string) (string, error) {
	...
	entries, ok := idx.Entries["sumboard"]
	if !ok {
		return "", fmt.Errorf("no entry for sumboard in repo index")
	}
	...
	if latest == nil {
		return "", fmt.Errorf("no valid version of sumboard chart found in repo index")
	}
	...
	return common.SbRepoURL + "/" + latest.URLs[0], nil
}
