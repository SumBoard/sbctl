...
const (
	...
	// orgID is the ID of the default organization created when Sumboard if first
...
// Sumboard is used for communicating with the Sumboard API
type Sumboard struct {
	h     HTTPClient
	token Token
	mu    sync.Mutex
...
// New returns an Sumboard client.
// The host is the hostname (with port) where the Sumboard API is hosted.
// The clientID and clientSecret are both required in order to create an application token.
func New(host, clientID, clientSecret string, opts ...Option) *Sumboard {
	a := &Sumboard{
		host:         host,
		clientID:     clientID,
...
func (a *Sumboard) GetOrgEmail(ctx context.Context) (string, error) {
...
func (a *Sumboard) SetOrgEmail(ctx context.Context, email string) error {
	org, err := a.getOrg(ctx)
	if err != nil {
		return fmt.Errorf("failed to get organization: %w", err)
	}
...
func (a *Sumboard) getOrg(ctx context.Context) (organization, error) {
...
func (a *Sumboard) setOrg(ctx context.Context, org organization) error {
	token, err := a.fetchToken(ctx)
	if err != nil {
...
func (a *Sumboard) fetchToken(ctx context.Context) (Token, error) {
	t := a.token
	if t != "" {
		return t, nil
	}
