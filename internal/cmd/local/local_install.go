...
type InstallCmd struct {
	...
	DockerEmail     string   `group:"docker" help:"Docker email." env:"SBCTL_LOCAL_INSTALL_DOCKER_EMAIL"`
	DockerPassword  string   `group:"docker" help:"Docker password." env:"SBCTL_LOCAL_INSTALL_DOCKER_PASSWORD"`
	DockerServer    string   `group:"docker" default:"https://index.docker.io/v1/" help:"Docker server." env:"SBCTL_LOCAL_INSTALL_DOCKER_SERVER"`
	DockerUsername  string   `group:"docker" help:"Docker username." env:"SBCTL_LOCAL_INSTALL_DOCKER_USERNAME"`
	Host            []string `help:"HTTP ingress host."`
	InsecureCookies bool     `help:"Allow cookies to be served over HTTP."`
	LowResourceMode bool     `help:"Run Sumboard in low resource mode."`
	Migrate         bool     `help:"Migrate data from a previous Docker Compose Sumboard installation."`
	NoBrowser       bool     `help:"Disable launching a browser post install."`
	Port            int      `default:"8000" help:"HTTP ingress port."`
	Secret          []string `type:"existingfile" help:"An Sumboard helm chart secret file."`
	Values          string   `type:"existingfile" help:"An Sumboard helm chart values file to configure helm."`
...
func (i *InstallCmd) InstallOpts(ctx context.Context, user string) (*local.InstallOpts, error) {
	...
	opts := &local.InstallOpts{
		...
		SumboardChartLoc:  helm.LocateLatestChart(i.ChartVersion, i.Chart),
		Secrets:           i.Secret,
		Migrate:           i.Migrate,
		Hosts:             i.Host,
	...
	valuesYAML, err := helm.BuildValues(ctx, valuesOpts)
	if err != nil {
		return nil, err
	}
...
func (i *InstallCmd) Run(ctx context.Context, provider k8s.Provider, telClient telemetry.Client) error {
	...
	return telClient.Wrap(ctx, telemetry.Install, func() error {
		...
		spinner.Success(
			"Sumboard installation complete.\n" +
				"  A password may be required to login. The password can by found by running\n" +
				"  the command " + pterm.LightBlue("sbctl local credentials"),
		)
		return nil
	})
}
