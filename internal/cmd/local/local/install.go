...
// Install handles the installation of Sumboard
func (c *Command) Install(ctx context.Context, opts InstallOpts) error {
	...
	if !c.k8s.NamespaceExists(ctx, airbyteNamespace) {
		...
		if err := c.k8s.NamespaceCreate(ctx, airbyteNamespace); err != nil {
			...
			return fmt.Errorf("unable to create sumboard namespace: %w", err)
		}
	...
	if opts.Migrate {
		c.spinner.UpdateText("Migrating sumboard data")
		if err := c.tel.Wrap(ctx, telemetry.Migrate, func() error { return migrate.FromDockerVolume(ctx, opts.Docker.Client, "airbyte_db") }); err != nil {
			pterm.Error.Println("Failed to migrate data from previous Sumboard installation")
			return fmt.Errorf("unable to migrate data from previous sumboard installation: %w", err)
		}
	}
...
func (c *Command) watchEvents(ctx context.Context) {
	...
	if err != nil {
		pterm.Warning.Printfln("Unable to watch sumboard events\n  %s", err)
		return
	}
	defer watcher.Stop()
...
func (c *Command) watchBootloaderLogs(ctx context.Context) {
	...
	for {
		...
		err := c.streamPodLogs(ctx, airbyteNamespace, airbyteBootloaderPodName, "sumboard-bootloader", since)
		if err == nil {
			break
		} else {
