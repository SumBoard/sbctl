...
// FromDockerVolume handles migrating the existing docker compose database into the sbctl managed k8s cluster.
func FromDockerVolume(ctx context.Context, dockerCli docker.Client, volume string) error {
	...
	// docker run -d -v sumboard_db:/var/lib/postgresql/data alpine:3.20 tail -f /dev/null
	...
	// docker cp [conCopy.ID]]:/$migratePGDATA/. ~/.sumboard/sbctl/data/sumboard-volume-db/pgdata
	dst := filepath.Join(paths.Data, "sumboard-volume-db", "pgdata")
	...
	if err := copyFromContainer(ctx, dockerCli, conCopy.ID, migratePGDATA+"/.", dst); err != nil {
		return fmt.Errorf("unable to copy sumboard db data from container %s: %w", conCopy.ID, err)
	}
	pterm.Debug.Println(fmt.Sprintf("Copied sumboard db data from container '%s' to '%s'", conCopy.ID, dst))
	...
	// docker run
	// -e POSTGRES_USER=docker -e POSTGRES_PASSWORD=docker -e POSTGRES_DB=sumboard -e PGDATA=/var/lib/postgresql/data \
	// -v ~/.sumboard/sbctl/data/sumboard-volume-db/pgdata:/var/lib/postgresql/data
	// postgres:13-alpine
	...
	// docker exec sumboard-sbctl-migrate psql -U docker -c "CREATE ROLE sumboard SUPERUSER CREATEROLE CREATEDB REPLICATION BYPASSRLS LOGIN PASSWORD 'sumboard'"
	// docker exec sumboard-sbctl-migrate psql -U sumboard postgres -c 'ALTER DATABASE sumboard RENAME TO "db-sumboard"'
	var (
		cmdPsqlRename = []string{"psql", "-U", "docker", "-d", "postgres", "-c", `ALTER DATABASE "sumboard" RENAME TO "db-sumboard"`}
		cmdPsqlUser   = []string{"psql", "-U", "docker", "-d", "postgres", "-c", `CREATE ROLE sumboard SUPERUSER CREATEROLE CREATEDB REPLICATION BYPASSRLS LOGIN PASSWORD 'sumboard'`}
	)
	// add a new database user to match the default helm user
	now := time.Now()
	pterm.Debug.Println("Adding Sumboard postgres user")
	...
	pterm.Debug.Println(fmt.Sprintf("Adding Sumboard postgres user completed in %s", time.Since(now)))
