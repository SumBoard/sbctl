...
func (d *DeploymentsCmd) deployments(ctx context.Context, k8sClient k8s.Client, spinner *pterm.SpinnerPrinter) error {
	if d.Restart == "" {
		...
		deployments, err := k8sClient.DeploymentList(ctx, sumboardNamespace)
		if err != nil {
	...
	if err := k8sClient.DeploymentRestart(ctx, sumboardNamespace, d.Restart); err != nil {
		pterm.Error.Println(fmt.Sprintf("Unable to restart sumboard deployment %s", d.Restart))
		return fmt.Errorf("unable to restart sumboard deployment: %w", err)
	}
