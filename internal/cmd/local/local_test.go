...
func TestCheckAirbyteDir(t *testing.T) {
	origDir := paths.Sumboard
	t.Cleanup(func() {
		paths.Sumboard = origDir
	})

	t.Run("no directory", func(t *testing.T) {
		paths.Sumboard = filepath.Join(t.TempDir(), "does-not-exist")
	...
	t.Run("directory with correct permissions", func(t *testing.T) {
		paths.Sumboard = filepath.Join(t.TempDir(), "correct-perms")
		if err := os.MkdirAll(paths.Sumboard, 0744); err != nil {
		...
		perms, err := os.Stat(paths.Sumboard)
		if err != nil {
	...
	t.Run("directory with higher permissions", func(t *testing.T) {
		paths.Sumboard = filepath.Join(t.TempDir(), "correct-perms")
		if err := os.MkdirAll(paths.Sumboard, 0777); err != nil {
			t.Fatal("unable to create test directory", err)
		}
		if err := os.Chmod(paths.Sumboard, 0777); err != nil {
			t.Fatal("unable to change permissions", err)
		}
		if err := checkAirbyteDir(); err != nil {
			t.Error("unexpected error", err)
	...
	t.Run("directory with incorrect permissions", func(t *testing.T) {
		paths.Sumboard = filepath.Join(t.TempDir(), "incorrect-perms")
		if err := os.MkdirAll(paths.Sumboard, 0200); err != nil {
			t.Fatal("unable to create test directory", err)
		}
		if err := os.Chmod(paths.Sumboard, 0200); err != nil {
			t.Fatal("unable to change permissions", err)
		}
