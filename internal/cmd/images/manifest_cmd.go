...
func (c *ManifestCmd) findImages(ctx context.Context) ([]string, error) {
	valuesYaml, err := helm.BuildValues(ctx, helm.ValuesOpts{
		ValuesFile: c.Values,
	})
	if err != nil {
		return nil, err
	...
	chartLoc := helm.LocateLatestChart(c.ChartVersion, c.Chart)
	return FindImagesFromChart(valuesYaml, chartLoc, c.ChartVersion)
}
...
func FindImagesFromChart(valuesYaml, chartName, chartVersion string) ([]string, error) {
	...
	client, err := helmlib.New(helm.ClientOptions(common.SbNamespace))
	if err != nil {
		return nil, err
	}

	err = client.AddOrUpdateChartRepo(repo.Entry{
		Name: common.SbRepoName,
		URL:  common.SbRepoURL,
	})
	if err != nil {
		return nil, err
	}
...
// It also looks for env vars in the config map that end with "_IMAGE".
...
func findAllImages(chartYaml string) []string {
	...
	for _, obj := range objs {
		...
		case *corev1.ConfigMap:
			if strings.HasSuffix(z.Name, "sumboard-env") {
