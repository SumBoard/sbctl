...
func TestProvider_Defaults(t *testing.T) {
	t.Run("DefaultProvider", func(t *testing.T) {
		...
		if d := cmp.Diff("sumboard-sbctl", DefaultProvider.ClusterName); d != "" {
			t.Errorf("ClusterName mismatch (-want +got):\n%s", d)
		}
		if d := cmp.Diff("kind-sumboard-sbctl", DefaultProvider.Context); d != "" {
	...
	t.Run("test", func(t *testing.T) {
		...
		if d := cmp.Diff("test-sumboard-sbctl", TestProvider.ClusterName); d != "" {
			t.Errorf("ClusterName mismatch (-want +got):\n%s", d)
		}
		if d := cmp.Diff("test-sumboard-sbctl", TestProvider.Context); d != "" {
			t.Errorf("Context mismatch (-want +got):\n%s", d)
		}
