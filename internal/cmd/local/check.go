...
// portAvailable returns a nil error if the port is available, or already is use by Sumboard, otherwise returns an error.
//
// This function works by attempting to establish a tcp listener on a port.
// If we can establish a tcp listener on the port, an additional check is made to see if Sumboard may already be
// bound to that port. If something besides Sumboard is using it, treat this as an inaccessible port.
